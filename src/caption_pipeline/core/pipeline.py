...
class Pipeline:
    ...
    def __init__(
        self, 
        error_handling: Literal["stop", "skip"] = "stop"
    ) -> None:
        """
    ...
    def on_error(
        self, 
        callback: Callable[[str, ImageContext | None, Exception], None]
    ) -> Self:
    ...
    def run(self, contexts: list[ImageContext]) -> list[ImageContext]:
        ...
        for step_idx, step in enumerate(self.steps):
            if not current_contexts:
                logger.warning(f"No contexts remaining before step {step.name()}")
                break
