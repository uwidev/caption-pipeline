...
class LlamaServer:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            log.debug(f"Server process started with PID: {self._pid}")

            # Start threads to log output if using pipes
            if stdout_dest == subprocess.PIPE:
                self._stdout_thread = threading.Thread(
                    target=self._log_output, args=(self.process.stdout, "STDOUT"), daemon=True
                )
                self._stdout_thread.start()

            if stderr_dest == subprocess.PIPE:
                self._stderr_thread = threading.Thread(
                    target=self._log_output, args=(self.process.stderr, "STDERR"), daemon=True
                )
            ...
            if self._wait_for_ready():
                self._is_ready = True
                log.info(f"Server is ready on port {self.config.port}")
                return True
    ...
    def stop(self) -> bool:
        ...
        if not self.process:
            log.debug("No server process to stop")
            self._is_ready = False
            return True

        log.info(f"Stopping llama-server (PID: {self._pid})...")

        try:
            # Check if process is still alive
            if self.process.poll() is not None:
                log.debug(f"Process already terminated with exit code: {self.process.poll()}")
                self._cleanup_process()
                return True
            ...
            if os.name != "nt":
                try:
                    ...
                    log.debug(f"Sent SIGTERM to process group {os.getpgid(self._pid)}")
                except ProcessLookupError:
                    log.debug("Process group already gone")
                    self._cleanup_process()
                    return True
                except OSError as e:
                    log.debug(f"OS error sending SIGTERM: {e}")
            ...
            try:
                stdout, stderr = self.process.communicate(timeout=self.config.shutdown_timeout)
                if stdout:
                    log.debug(f"Server stdout: {stdout[:500]}")
                if stderr:
                    log.debug(f"Server stderr: {stderr[:500]}")
            except subprocess.TimeoutExpired:
                log.warning("Server didn't shut down gracefully, forcing kill")
                if os.name != "nt":
                    try:
            ...
            if e.errno == 9:  # Bad file descriptor
                log.debug("Process already terminated (bad file descriptor)")
                self._cleanup_process()
                return True
            log.error(f"OS error stopping server: {e}")
            self._cleanup_process()
            return False
        except Exception as e:
        ...
        if self.process:
            try:
                ...
                if e.errno != 9:  # 9 = Bad file descriptor
                    log.debug(f"Error killing process: {e}")
            finally:
                self.process = None
    ...
    def _wait_for_ready(self, timeout: int | None = None) -> bool:
        ...
        log.debug(f"Waiting for server to become ready (timeout: {timeout}s)")

        while time.time() - start_time < timeout:
            # Check if process is still alive
            if self.process and self.process.poll() is not None:
                log.error(f"Server process died (exit code: {self.process.poll()})")
                return False
            ...
            for endpoint in endpoints:
                try:
                    ...
                    if 200 <= response.status_code < 300:
                        log.debug(
                            f"Server ready (endpoint: {endpoint} returned {response.status_code})"
                        )
                        return True
    ...
    def _validate_config(self) -> bool:
        ...
        if not self.config.model_path:
            log.error("Model path not set in configuration")
            return False

        if not self.config.model_path.exists():
            log.error(f"Model file not found: {self.config.model_path}")
            return False

        if not self.config.mmproj_path:
            log.error("MMProj path not set in configuration")
            return False

        if not self.config.mmproj_path.exists():
            log.error(f"MMProj file not found: {self.config.mmproj_path}")
            return False
    ...
    def _get_output_destinations(self):
        ...
        if self.config.log_file:
            try:
                ...
                log.warning(f"Could not open log file: {e}, falling back to DEBUG logging")
        ...
        try:
            for line in iter(pipe.readline, ""):
                if line:
                    ...
                    log.debug(f"[llama-server {name}] {line.rstrip()}")
        except Exception as e:
    ...
    def _get_failure_message(self) -> str:
        ...
        for line in self._startup_logs:
            if any(
                ...
                for keyword in ["error", "fail", "exception", "cuda", "memory"]
            ):
