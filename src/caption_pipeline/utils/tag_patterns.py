...
PRE_CATEGORIZE_PATTERNS: Final[list[tuple[re.Pattern[str], str]]] = [
    # ---- Rating: tags containing censor/censoring ----
    # Catches: censor, censored, uncensored, censor_bar, pixel_censor, etc.
    (re.compile(r".*censor.*"), "rating"),
    # ---- Actions (prefix verb_object) ----
    # Catches: holding_sword, lifting_skirt, looking_at_viewer, etc.
    (
        re.compile(
            r"^(holding|wearing|removing|taking_off|putting_on|lifting|pulling|pushing|carrying|dragging|riding|driving|flying|swimming|diving|jumping|leaping|climbing|kicking|punching|grabbing|throwing|catching|pointing_at|reaching_for|looking_at|looking_to|staring_at|glaring_at|waving_at|talking_to|shouting_at|whispering_to|kissing|hugging|cuddling|patting|stroking|groping|spanking|biting|licking|sucking|shooting|stabbing|slashing|cutting|breaking|picking_up|putting_down|opening|closing|washing|cleaning|cooking|eating|drinking|playing|singing|dancing)_"
        ),
        "action",
    ),
    # ---- Actions (gaze) ----
    ...
    (
        re.compile(
            r".*_(hair|eyes|tail|ears|horns|wings|skin|fur|claws|hands|feet|legs|arms|breasts|chest|back|neck|head|face|mouth|nose|chin|thighs|calves|toes|fingers|paws|hooves|antlers|tentacles|scales|feathers|eye)$"
        ),
        "body parts",
    ),
